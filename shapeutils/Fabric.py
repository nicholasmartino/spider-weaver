...
from ShapeTools import Shape, SpatialAnalyst, divide_line_by_count
...
class Neighbourhood:
    ...
    def export_centers_feather(self, merge_existing=True):
        """
        Write laz file from neighbourhood centers
        :param merge_existing: If true it will try to read an existing centers.laz file
        :return:
        """
        self.update_lng_lat_ids()
        ...
        if merge_existing:
            try:
                gdf = pd.concat([gpd.read_feather('centers.laz'), self.buffer_centers()])
            except:
    ...
    def assign_gradient_fsr(self, fsr_range, max_dist=500):
        ...
        if len(self.centers) == 0:
            try: self.centers = gpd.read_feather('centers.laz')
            except: pass
